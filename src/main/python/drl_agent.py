...
# Bridge URL for realtime logs
BRIDGE_LOG_URL = "http://localhost:4000/log"
...
def main_loop():
    ...
    try:
        requests.post(BRIDGE_LOG_URL, json={"level":"info","msg":"Python agent started","source":"python-agent"}, timeout=0.5)
    except:
        pass
    ...
    while True:
        if os.path.exists(DONE_FILE):
            ...
            try:
                requests.post(BRIDGE_LOG_URL, json={"level":"info","msg":"Stop signal received","source":"python-agent"}, timeout=0.5)
            except:
                pass
            os.remove(DONE_FILE)
            break
        ...
        if os.path.exists(STATE_FILE):
            try:
                ...
                try:
                    ...
                    pass
                # Log action to bridge
                try:
                    requests.post(BRIDGE_LOG_URL, json={"level":"debug","msg":f"Action chosen: {action}","source":"python-agent"}, timeout=0.5)
                except:
                    pass
            except:
                pass
            ...
            try:
                ...
                if last_state is not None:
                    ...
                    if loss > 0:
                        losses.append(loss)
                    try:
                        msg = f"Reward {reward:.2f} | Loss {loss:.4f}" if loss > 0 else f"Reward {reward:.2f}"
                        requests.post(BRIDGE_LOG_URL, json={"level":"debug","msg":msg,"source":"python-agent"}, timeout=0.5)
                    except:
                        pass
